...
- 딥러닝 프레임워크를 구성하는 각 요소들의 기본 개념을 이해할 수 있다. 
- 이 책에 코드 구현은 없지만 numpy로 짜여진 코드가 있다면 첨부하겠다.
- 각 챕터의 순서는 책의 순서를 따르지만, 챕터 내 설명은 재구성하였다.
- 이론적인 상세한 설명은 통계 전공의 지식을 바탕으로 첨언하였다.
...
- 퍼셉트론의 한계 <br/>
퍼셉트론은 일종의 **이진 분류기**다. 어떤 **선형식** $ w \ cdot x $ 을 기준으로  $ w \ cdot x >0 $ 이면  1 아니면 0을 반환한다. 여기서 $ w $ 는 가중치다. 한 층의 퍼셉트론이 갖는 대표적인 한계가 선형으로 분류 되지 않는 XOR 문제다. $ x_1 $ , $ x_2 $ 이 1 또는 0을 갖는 논리값이라고 할 때, XOR은 $ 중 $ x_1 $ , $ x_2 $ 중 하나만 1일 때 1을 반환하고, 나머지의 경우 0을 반환하는 논리회로다. 아래 그림과 같이 XOR은 하나의 직선으로 두 색깔의 점을 분류할 수 없다. 즉 퍼셉트론으로 XOR 분류 문제를 해결할 수 없다.
...
- class가 3개인 값의 예를 들어보자. 첫번째 관측치의 label이 [1, 0, 0] 벡터라 할 때, 이 벡터는 참 분포인 $ p(x) $ 에 해당한다. 그리고 학습을 통해 얻은 추정치 벡터가 [ 0.7, 0.2, 0.1 ] 이라 할 때 이 벡터는 후보 분포인 $ q(x) $ 에 해당한다. 따라서 첫 번째 관측치에 대한 크로스엔트로피는 - {1xln(0.7) + 0xln(0.2)+0xln(0.1)}이다.

		 
1.2. 관점2. 이항분포 MLE(최대 우도) <br/>

- 엔트로피란 마이너스 로그우도함수 ( $ - ln f(x) $ ) 의 기대값이다. 즉, 로그우도 함수를 최대화하는 문제는 엔트로피가 최소화하는 문제와 동치이다.
- 다항분포는 k개의 카테고리가 $ p_1,p_2, ..., p_k $ 의 확률로 발생할 수 있고, 각 카테고리의 사건 발생 횟수가 $ x_1, x_2 ... , x_k $ 일 때의 분포다. 단, 총 독립시행 횟수 $ n $ 은 각 카테고리에서의 발생횟수의 합인 $x_1+x_2+...+x_k $ 다. 딥러닝에서의 손실함수를 여기에 대입해서 생각해보면, k개 카테고리를 가진 사건을 1번 독립시행을 한 다항 분포의 경우다.
