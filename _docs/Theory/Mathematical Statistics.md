...
젠센 부등식에 의해 <br/>
$ E_p(\phi(R)) \leq \phi(E_p(R)) = -ln(E_p(R)) = -ln1 = 0 $  <br/>
...
KL 발산은 엔트로피 개념을 이용해 동일 확률변수 X에 대해 분포 간 유사성을 측정한 것이다.
> <center> $ D_{KL} (P \left \|  \right \| Q) = E_p( ln P(x) - ln Q(x) ) = \int p(x) ln \frac{p(x)}{q(x)} d\mu $ <center/>    
