...
<br/>
사후 모수를 $ \theta^{post} $ 라 하자. 사후 모수의 표본 $ \theta^{post}_1, ..., \theta^{post}_n $ 을 MCMC 샘플링해 사후 표본을 뽑는다. 표본의 기대값을 $ \hat{\theta^{post}} $ 의 추정치로 사용한다. 위에서 설명한 (1)과 (2)를 종합하여 MCMC 베이지안 사후표본 샘플링을 설명하자면, (2)에 의해 k+1번째 표본인 $ \theta%^{post}_{k+1} $ 는 직전에 뽑힌 k번째 표본인 $ \theta^{post}_k $ 에 의존하여 샘플링된다. 또한 (1)에 의해 사후표본의 평균 $ \bar{\theta_{post}_n} $ 는 샘플의 크기가 커짐에 따라 참값 $ \theta_{post} $ 에 근사한다.
<br/>
