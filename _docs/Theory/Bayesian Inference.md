...
<br/>
(1)과 (2)를 종합하여 MCMC 베이지안 샘플링을 설명하면 다음과 같다. 사후 모수의 표본 샘플링 시 직전 사후 모수 후보값에 의해 현재 사후 모수 후보값을 샘플링하며, 사후모수의 샘플평균은 사후모수의 모평균에 근사한다. 다만, 직전 후보값이 현재 후보값으로 업데이트될 지 결정하는 데에 채택확률 개념이 개입된다. 이 채택확률에 따라 알고리즘이 분화되는데 대표적으로 **메트로폴리스 헤이스팅스 알고리즘**과 **깁스 샘플링**이 있다.
