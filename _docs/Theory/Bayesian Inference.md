...
표본수가 커짐에 따라 랜덤표본에 어떤 함수 $ h $ 를 취한것의 '평균'이 모집단에 $ h $ 를 취한 것의 ''에 근접하는 것을 몬테카를로 방법이라 부른다. <br/>
임의의 함수를 $ h $, 확률변수 $ X $ 의 확률밀도함수를 $ f $, 표본 $X_1, X_2, ..., X_n $ 이 i.i.d. 랜덤 샘플로부터 뽑혔을 때 몬테카를로 방법을 수식화하면 다음과 같다.
