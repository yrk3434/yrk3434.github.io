...
여기서 메트로폴리스 헤이스팅스 알고리즘은 $ g $ 가 symmetric한 경우다. 즉, $ g(\theta^{(t)}|\theta^*) = g(\theta^*|\theta^{(t)}). $ 따라서 <br/> 
<center> $ R(\theta^*, \theta^{(t)} ) = \frac{ f(\theta^*) }{ f(\theta^{(t)}) } $ <center/> <br/>
